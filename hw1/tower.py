...
class Node:
    ...
    # Return a list of states arising from legal moves from current node:
...
# Search strategy based on user input argument "method":
def nodeSelect(frng, mthd):
    if mthd == "bestfirst":
...
# Overall graph search function:
def towerOfHanoi(num_pegs, num_disks, method):
    ...
    while(True):
        ...
        # Expand node from the fringe:
        node = nodeSelect(fringe, method)
        fringe.remove(node)
        explored.add(node)
        ...
        for state_after_move in node.possible_moves():
            ...
            if not ((state_after_move in [n.state for n in explored]) or (state_after_move in [n.state for n in fringe])):
                ...
                child_node.eval = sum(state_after_move[-1]) - child_node.pathCost
...
if __name__ == '__main__':
    ...
    searchStrategy = input("Enter the search strategy (\"bfs\", \"dfs\", or \"bestfirst\"): ")
    solution_path, solution_cost, search_iters = towerOfHanoi(pegs, disks, searchStrategy)
