...
'''
Assert a new fact (or rule) into KB; also assert other facts/rules INFERRED from comparing this new fact (or rule)
with the existing rules (or facts)
'''
...
'''
Given a list of patterns, like [['color', '?x', 'blue'], ['inst', 'bigbox', '?w']], return the bindings in KB that
hold when those patterns are true
'''
def ask(patterns, fb):
    bindings_list = []
...
if __name__ == '__main__':
    ...
    query = [['color', '?x', 'blue'], ['color', '?y', 'red'], ['flat', '?z'], ['inst', 'bigbox', '?w']]
    bl = ask(query, FB)
    print "For Ask query: "
    print query
    print "\nthe answers are: "
